    ...
    addTeamBtn.addEventListener('click', () => {
      ...
      if (!name) return alert('אנא הכנס שם צוות');
      if (teams.includes(name)) return alert('שם הצוות כבר קיים');
      teams.push(name);
      scores[name] = 0;
      teamNameInput.value = '';
      renderTeams();
    ...
    function renderTeams() {
      ...
      teams.forEach((team, i) => {
        ...
        <button onclick="removeTeam(${i})" style="background:#b33939; color:#fff; border:none; border-radius:4px; cursor:pointer;">הסר</button>
      `;
    ...
    window.removeTeam = function (i) {
      if (gameStarted) {
        alert('לא ניתן להסיר צוות במהלך משחק');
        return;
      }
    ...
    startGameBtn.addEventListener('click', () => {
      ...
      // alert(`המשחק מתחיל! תור צוות: ${teams[currentTeamIndex]}`);
    });
    ...
    function prepareItems() {
      if (phase === 'songs') {
        ...
        // בחר רק חפצים (או בעלי חיים) שלא הוצגו עדיין
    ...
    function showWinnersOverlay() {
      ...
      const winners = teams.filter(t => scores[t] === maxScore);
    ...
    continueToSongsBtn.addEventListener('click', () => {
      ...
      backgroundMusic.pause();
      backgroundMusic.currentTime = 0;
      alert(`המשחק בשלב השירים התחיל! תור צוות: ${teams[currentTeamIndex]}`);
    });
    ...
    roundTimeInput.addEventListener('change', () => {
      ...
      if (val < 10) {
        alert('יש להכניס זמן של לפחות 10 שניות');
